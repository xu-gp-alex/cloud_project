        ...
        for(int i = 0; i < 4; i++) {
            if(machine_by_cpus[i].size() != 0) {
                ...
                if (cpu_util > 0.8) {
                    ...
                    while (idle_index < off[i].size() && theoretical_util > 0.8) {
                    ...
                    unsigned idle_index = 0;
                    while(idle_index < idle_size && idle_size > (double) machine_by_cpus[i].size() * 0.4) {
                        MachineId_t id = idle[i][idle_index];
                        if(!changing_state.count(id)) {
                            Machine_SetState(id, OFF_S_STATE);
                            changing_state[id] = {RUNNING_S_STATE, OFF_S_STATE, false, 0};
                            idle_size--;
                        }
                        idle_index++;
                    }
